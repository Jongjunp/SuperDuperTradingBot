...
class Env:
    ...
    def load_data(self):
        ...
        self.tmp = np.array(self.dataset[0], dtype=np.float)[:, 2::1]
        return
    ...
    def sell(self, code):
        if self.stock > 0:
            self.stock -= 1
            self.balance += self.stock_value * .999
        return
    ...
    def next_step(self, action):
        ...
        obs = self.make_data()
        tmp = self.balance + self.stock * self.batch[58, 3] * .999
        reward = self.pred_account - tmp
        self.pred_account = tmp
        self.pred_benefit = self.pred_account - self.initial_balance

        done = True if self.date > 500 else False
        print(self.balance, self.stock, self.pred_account)
    ...
    def make_data(self):
        # 처음 시작할 때 batch 생성
        if self.date == 59:
            self.batch = self.tmp[0:60:1, :]

        # 이전 데이터 밀어내기
        self.stock_value = self.batch[59, 0]
        self.batch = np.vstack((self.batch[1:, :], self.tmp[self.date, :]))

        return np.vstack((np.hstack((self.batch[:, :3] * np.full((60, 3), 1/self.batch[59, 1]),
                          self.batch[:, 3:] * np.full((60, 1), 1/72800000*3))),
